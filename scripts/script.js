...
var parsedStationData = JSON.stringify(stations);
parsedStationData = parsedStationData.split(',');
//HTML functions
...
function getElevation() {
    var config = {
        method: 'get',
        url: `https://nationalmap.gov/epqs/pqs.php?y=${latitude}&x=${longitude}&units=Meters&output=xml`
    };
    axios(config)
...
function getCounty() {
    $.ajax({
        url: `https://geo.fcc.gov/api/census/area?lat=${latitude}&lon=${longitude}&censusYear=2020&format=json`
,
        dataType: 'jsonp',
        success: function(response) {
            console.log(JSON.stringify(response))
            county = response.result.county_name[0];
          alert(county)
...
function getSeaRiseRate(isGeocoded){
	if(!isGeocoded){
		getCounty();
	}
  for(let i = 1; i<parsedStationData.length; i+=4){
    alert(parsedStationData[i])
    if(parsedStationData[i] == county)
    {
      return parsedStationData[i+2]//returns SLR rate as string. TODO: convert to int
    }
  }
}
